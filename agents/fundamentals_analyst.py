...
from utils.data_tools import get_financials, get_detailed_financials
...
class FundamentalsAnalyst(BaseAgent):
    ...
    def analyze(self, ticker):
        ...
        if key_metrics.get("Market Cap") == "N/A" and not is_crypto:
            data_warning = "\n**WARNING: Key financial data is missing. The ticker symbol might be incorrect (e.g., 'APPL' instead of 'AAPL'). Please verify the ticker.**\n"
        ...
        if is_crypto:
            ...
            **Note:** Traditional financial statements (Income, Balance Sheet) do not apply to cryptocurrencies.
            ...
            **Cash Flow (Recent):**
            ...
            If data is missing, explicitly state that the ticker might be invalid in the report.
