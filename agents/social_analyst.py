...
class SocialAnalyst(BaseAgent):
    ...
    def analyze(self, ticker):
        ...
        # Try fetching real social signals via Tavily
        ...
        if tavily_api_key:
            try:
                ...
                # Search for Twitter/X sentiment (via Google search proxy)
        ...
        # Fallback to simulation if Tavily failed or no key
        if not social_data:
        ...
        Your job is to gauge the public sentiment and 'buzz' around a stock based on social media discussions (Reddit, Twitter, Forums).
        **IMPORTANT: Write the entire report in Thai language.**
        ...
        Please provide:
        1. A summary of what retail investors are discussing (mention specific platforms if available).
        2. An analysis of the 'Hype' vs 'Fear' levels.
        3. A final social sentiment verdict (BULLISH/BEARISH/NEUTRAL).
        
        **IMPORTANT:** Cite the source (Reddit/Twitter) if possible.
